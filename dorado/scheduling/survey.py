...
from astropy.coordinates import ICRS, ITRS, SkyCoord, SkyOffsetFrame, TEME
...
class SurveyModel():
    def __init__(self,
        ...
        self.exposures_per_orbit = int(
            (self.orbital_period / exposure_time).to_value(u.dimensionless_unscaled))
        self.time_steps = int(
            (self.orbital_period / self.time_step_duration).to_value(u.dimensionless_unscaled))

        self.healpix = HEALPix(nside=32, order='nested', frame=ICRS())
        """Base HEALpix resolution for all calculations."""
        
        if centers is None:
            self.centers = self.healpix.healpix_to_skycoord(np.arange(self.healpix.npix))
    ...
    def get_footprint_grid(self):
        ...
        xyz = np.moveaxis(
            self.get_footprint_polygon(
                self.centers[:, None], self.rolls[None, :]).cartesian.xyz.value, 0, -1)
        query_polygon = partial(
            hp.query_polygon, self.healpix.nside, nest=(self.healpix.order == 'nested'))
