...
def main(args=None):
    ...
    prob = rasterize(skymap, nside_to_level(uvexgrid.healpix.nside))['PROB']
    if uvexgrid.healpix.order == 'ring':
        prob = prob[uvexgrid.healpix.ring_to_nested(np.arange(len(prob)))]
    ...
    log.info('loading tiling')
    coords = np.loadtxt(args.tiles)
    centers = SkyCoord(coords[:, 1] * u.deg, coords[:, 2] * u.deg)
    ...
    pixel_observed = np.asarray(m.binary_var_list(uvexgrid.healpix.npix))
    ...
    indices = [[] for _ in range(uvexgrid.healpix.npix)]
    with tqdm(total=len(centers)) as progress:
        for i, grid_i in enumerate(uvexgrid.get_footprint_grid(centers)):
